...
const obtenerListaDeTarjetaCredito = async () => {
  try {
    const listaTarjetaCredito =
      await TarjetaCredito.obtenerListaDeTarjetaCredito();
...
const obtenerTarjetaCreditoIndividual = async (tarjetaCreditoId) => {
  try {
    const tarjetaCredito = await TarjetaCredito.obtenerTarjetaCreditoIndividual(
      tarjetaCreditoId
    );
...
const registrarTarjetaCredito = async (nuevaTarjetaCredito) => {
  try {
    ...
    const idTarjetaCreditoNueva = await obtenerUltimoId(
      "codigo_metodo",
      "metodo_pago_tc"
    );
    ...
    for (let i = 0; i < listaApuesta.length; i++) {
      console.log("AQUI");
      console.log(listaApuesta[i]);
      console.log(listaApuesta[i].participacion.codigo_participacion);
