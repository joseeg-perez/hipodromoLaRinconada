...
const obtenerListaDeParticipacionImplementos = async () => {
  try {
    const listaParticipacionImplementos =
      await ParticipacionImplemento.obtenerListaDeParticipacionImplementos();
...
const obtenerParticipacionImplementoIndividual = async (
  ...
  try {
    const participacionImplemento =
      await ParticipacionImplemento.obtenerParticipacionImplementoIndividual(
        participacionImplementoId
      );
...
const registrarParticipacionImplemento = async (
  ...
  try {
    await ParticipacionImplemento.registrarParticipacionImplemento(
      nuevaParticipacionImplemento
    );
...
const actualizarParticipacionImplemento = async (
  ...
  try {
    await ParticipacionImplemento.actualizarParticipacionImplemento(
      participacionImplementoId,
      cambios
    );
...
const borrarParticipacionImplemento = async (participacionImplementoId) => {
  try {
    await ParticipacionImplemento.borrarParticipacionImplemento(
      participacionImplementoId
    );
  } catch (error) {
    throw error;
