...
const obtenerListaDeReglas = async () => {
  ...
  try {
    ...
    if (rows.length === 0) httpError.noRegistrado("ningun tipo de regla");
...
const obtenerReglaIndividual = async (reglaId) => {
  ...
  try {
    const { rows } = await dbConnection.query(query);
    if (rows.length === 0) httpError.idNoEncontrado("La regla", reglaId);
...
const registrarRegla = async (nuevaRegla) => {
  const { nombreRegla, descripcionRegla } = nuevaRegla;
...
const actualizarRegla = async (reglaId, cambios) => {
  const { nombreRegla, descripcionRegla } = cambios;
  ...
  const query = {
    ...
    values: [nombreRegla, descripcionRegla, reglaId],
  };

  try {
    const { rowCount } = await dbConnection.query(query);
    if (rowCount === 0) httpError.idNoEncontrado("La regla", reglaId);
