...
const obtenerIGPFA = async () => {
    ...
    try {
        ...
        if (resGradas.rows.length === 0)
            httpError.noRegistrado("ninguna grada");
        
        const resPatios = await dbConnection.query(queryPatios);
        if (resPatios.rows.length === 0)
            httpError.noRegistrado("ningun patio");


        const resFilas = await dbConnection.query(queryFilas);
        if (resFilas.rows.length === 0)
            httpError.noRegistrado("ninguna fila");

        const resAsientos = await dbConnection.query(queryAsientos);
        if (resAsientos.rows.length === 0)
            httpError.noRegistrado("ningun asiento");

        const listaIGPFA = [resInfraestructuras.rows, resGradas.rows, resPatios.rows, resFilas.rows, resAsientos.rows];
