...
const actualizarColorStudVestimenta = async (colorStudVestimentaId, cambios) => {
    const { 
        ...
        try {
            ...
            if (rowCount === 0)
                httpError.idNoEncontrado("El color de la vestimenta del stud", entrenadorId);
...
const borrarColorStudVestimenta = async (colorStudVestimentaId) => {
    ...
    try {
        const { rowCount } = await dbConnection.query(query);        
        if (rowCount === 0)
            httpError.idNoEncontrado("El color de la vestimenta del stud", colorStudVestimentaId);
