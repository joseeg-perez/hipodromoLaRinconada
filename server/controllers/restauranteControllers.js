...
const obtenerListaDeRestaurantes = async (req, res) => {
  try {
    const listaRestaurantes =
      await restauranteService.obtenerListaDeRestaurantes();
...
const obtenerRestauranteIndividual = async (req, res) => {
  ...
  try {
    const restaurante = await restauranteService.obtenerRestauranteIndividual(
      restauranteId
    );
...
const registrarRestaurante = async (req, res) => {
  ...
  try {
    const restauranteCreado = await restauranteService.registrarRestaurante(
      nuevoRestaurante
    );
    res
      .status(200)
      .send({
        status: "OK",
        data: `Se ha registrado el restaurante '${restauranteCreado}' de forma satisfactoria.`,
      });
  } catch (error) {
    res
      .status(error?.status || 500)
...
const actualizarRestaurante = async (req, res) => {
  ...
  try {
    const restauranteActualizado =
      await restauranteService.actualizarRestaurante(restauranteId, body);
    res.send({
      status: "OK",
      data: `Se ha actualizado la informacion del restaurante '${restauranteActualizado}' de forma satisfactoria.`,
    });
  } catch (error) {
    res
      .status(error?.status || 500)
...
const borrarRestaurante = async (req, res) => {
  ...
  try {
    ...
    res
      ...
      .send({
        status: "OK",
        data: `El restaurante con el id '${restauranteId}' se ha eliminado con exito.`,
      });
  } catch (error) {
    res
      .status(error?.status || 500)
