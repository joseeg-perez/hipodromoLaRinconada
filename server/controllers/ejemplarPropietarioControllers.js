...
const obtenerListaDeEjemplarPropietarios = async (req, res) => {
  try {
    const listaEjemplarPropietarios =
      await ejemplarPropietarioService.obtenerListaDeEjemplarPropietarios();
...
const obtenerEjemplarPropietarioIndividual = async (req, res) => {
  ...
  try {
    const ejemplarPropietario =
      await ejemplarPropietarioService.obtenerEjemplarPropietarioIndividual(
...
const actualizarEjemplarPropietario = async (req, res) => {
  ...
  try {
    await ejemplarPropietarioService.actualizarEjemplarPropietario(
      ejemplarPropietarioId,
      body
    );
    res.send({
      status: "OK",
      data: `El ejemplar asociado a un propietario fue actualizado con exito`,
    });
  } catch (error) {
    res
      .status(error?.status || 500)
...
const borrarEjemplarPropietario = async (req, res) => {
  ...
  try {
    await ejemplarPropietarioService.borrarEjemplarPropietario(
      ejemplarPropietarioId
    );
    res.status(200).send({
      status: "OK",
      data: `El ejemplarPropietario con el id '${ejemplarPropietarioId}' se ha eliminado con exito.`,
    });
  } catch (error) {
    res
      .status(error?.status || 500)
