...
const registrarJinete = async (req, res) => {
  ...
  try {
    ...
    res
      ...
      .send({
        status: "OK",
        data: `Se ha registrado el jinete '${jineteCreado}' de forma satisfactoria.`,
      });
  } catch (error) {
    res
      .status(error?.status || 500)
...
const actualizarJinete = async (req, res) => {
  ...
  try {
    const jineteActualizado = await jineteService.actualizarJinete(
      jineteId,
      body
    );
    res.send({
      status: "OK",
      data: `Se ha actualizado la informacion del jinete '${jineteActualizado}' de forma satisfactoria.`,
    });
  } catch (error) {
    res
      .status(error?.status || 500)
...
const borrarJinete = async (req, res) => {
  ...
  try {
    ...
    res
      ...
      .send({
        status: "OK",
        data: `El jinete con el id '${jineteId}' se ha eliminado con exito.`,
      });
  } catch (error) {
    res
      .status(error?.status || 500)
