...
const obtenerListaDePuestoCaballos = async (req, res) => {
    try {
        const listaPuestoCaballos =  await puestoCaballoService.obtenerListaDePuestoCaballos();
...
const obtenerPuestoCaballoIndividual = async (req, res) => { 
    ...
    try {
        const puestoCaballo = await puestoCaballoService.obtenerPuestoCaballoIndividual(puestoCaballoId);
...
const borrarPuestoCaballo = async (req, res) => {
    ...
    try {
        ...
        res.status(200).send({ status: "OK", data: `El puesto para caballo con el id '${puestoCaballoId}' se ha eliminado con exito.` });
    } catch (error) {
        res
        .status(error?.status || 500)
        .send({ status: "FAILED", data: {error: error?.message || error} });
