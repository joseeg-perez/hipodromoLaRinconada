...
const actualizarVeterinario = async (req, res) => {
  ...
  try {
    const veterinarioActualizado =
      await veterinarioService.actualizarVeterinario(veterinarioId, body);
    res.send({
      status: "OK",
      data: `Se ha actualizado la informacion del veterinario '${veterinarioActualizado}' de forma satisfactoria.`,
    });
  } catch (error) {
    res
      .status(error?.status || 500)
