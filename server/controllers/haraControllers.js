...
const registrarHara = async (req, res) => {
  const { nombreHara, fkLugar } = req.body;
  ...
  try {
    ...
    res
      ...
      .send({
        status: "OK",
        data: `Se ha registrado el hara '${haraCreada}' de forma satisfactoria.`,
      });
  } catch (error) {
    res
      .status(error?.status || 500)
...
const actualizarHara = async (req, res) => {
  ...
  try {
    ...
    res.send({
      status: "OK",
      data: `Se ha actualizado la informacion del hara '${haraActualizada}' de forma satisfactoria.`,
    });
  } catch (error) {
    res
      .status(error?.status || 500)
...
const borrarHara = async (req, res) => {
  ...
  try {
    ...
    res
      ...
      .send({
        status: "OK",
        data: `El hara con el id '${haraId}' se ha eliminado con exito.`,
      });
  } catch (error) {
    res
      .status(error?.status || 500)
