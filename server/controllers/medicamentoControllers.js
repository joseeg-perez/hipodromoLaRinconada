...
const obtenerListaDeMedicamentos = async (req, res) => {
    try {
        const listaMedicamentos =  await medicamentoService.obtenerListaDeMedicamentos();
...
const obtenerMedicamentoIndividual = async (req, res) => {
    ...
    try {
        if (!medicamentoId)
            return(httpError.idVacio(res, ":medicamentoId"));
...
const registrarMedicamento = async (req, res) => {
    ...
    try {
        ...
        res.status(200).send({ status: "OK", data: `Se ha registrado el medicamento '${medicamentoCreado}' de forma satisfactoria.` });
    } catch (error) {
        res
        .status(error?.status || 500)
        .send({ status: "FAILED", data: { error: error?.message || error } });
