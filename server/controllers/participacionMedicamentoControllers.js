...
const obtenerListaDeParticipacionMedicamentos = async (req, res) => {
  try {
    const listaParticipacionMedicamento =
      await participacionMedicamentoService.obtenerListaDeParticipacionMedicamentos();
...
const obtenerParticipacionMedicamentoIndividual = async (req, res) => {
  ...
  try {
    const participacionMedicamento =
      await participacionMedicamentoService.obtenerParticipacionMedicamentoIndividual(
...
const registrarParticipacionMedicamento = async (req, res) => {
  const { fkMedicamento, fkParticipacion } = req.body;
  ...
  try {
    await participacionMedicamentoService.registrarParticipacionMedicamento(
    ...
    res
      ...
      .send({
        status: "OK",
        data: `Se ha registrado el medicamento en la participacion de forma satisfactoria.`,
      });
  } catch (error) {
    res
      .status(error?.status || 500)
...
const actualizarParticipacionMedicamento = async (req, res) => {
  ...
  try {
    await participacionMedicamentoService.actualizarParticipacionMedicamento(
      participacionMedicamentoId,
      body
    );
    res.send({
      status: "OK",
      data: `Se ha actualizado la informacion del medicamento asociada a una participacion de forma satisfactoria.`,
    });
  } catch (error) {
    res
      .status(error?.status || 500)
...
const borrarParticipacionMedicamento = async (req, res) => {
  ...
  try {
    await participacionMedicamentoService.borrarParticipacionMedicamento(
      participacionMedicamentoId
    );
    res
      .status(200)
      .send({
        status: "OK",
        data: `El medicamento en la participacion con el id '${participacionMedicamentoId}' se ha eliminado con exito.`,
      });
  } catch (error) {
    res
      .status(error?.status || 500)
