...
const obtenerListaDeParticipacionImplementos = async (req, res) => {
  try {
    const listaParticipacionImplemento =
      await participacionImplementoService.obtenerListaDeParticipacionImplementos();
...
const obtenerParticipacionImplementoIndividual = async (req, res) => {
  ...
  try {
    const participacionImplemento =
      await participacionImplementoService.obtenerParticipacionImplementoIndividual(
...
const registrarParticipacionImplemento = async (req, res) => {
  const { fkImplemento, fkParticipacion } = req.body;
  ...
  try {
    await participacionImplementoService.registrarParticipacionImplemento(
    ...
    res
      ...
      .send({
        status: "OK",
        data: `Se ha registrado el implemento en la participacion de forma satisfactoria.`,
      });
  } catch (error) {
    res
      .status(error?.status || 500)
...
const actualizarParticipacionImplemento = async (req, res) => {
  ...
  try {
    await participacionImplementoService.actualizarParticipacionImplemento(
      participacionImplementoId,
      body
    );
    res.send({
      status: "OK",
      data: `Se ha actualizado la informacion del implemento asociado a una participacion de forma satisfactoria.`,
    });
  } catch (error) {
    res
      .status(error?.status || 500)
...
const borrarParticipacionImplemento = async (req, res) => {
  ...
  try {
    await participacionImplementoService.borrarParticipacionImplemento(
      participacionImplementoId
    );
    res
      .status(200)
      .send({
        status: "OK",
        data: `El implemento en la participacion con el id '${participacionImplementoId}' se ha eliminado con exito.`,
      });
  } catch (error) {
    res
      .status(error?.status || 500)
