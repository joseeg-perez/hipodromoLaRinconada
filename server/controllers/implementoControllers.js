...
const obtenerListaDeImplementos = async (req, res) => {
    try {
        const listaImplementos =  await implementoService.obtenerListaDeImplementos();
...
const obtenerImplementoIndividual = async (req, res) => {
    ...
    try {
        if (!implementoId)
            return(httpError.idVacio(res, ":implementoId"));

        if (isNaN(implementoId) || implementoId === ' ')
            return(httpError.idInvalido(res, ":implementoId"));

        const implemento = await implementoService.obtenerImplementoIndividual(implementoId);
