...
const obtenerListaDeMotivosDeRetiro = async (req, res) => {
  try {
    const listaDeMotivosDeRetiros =
      await motivoRetiroService.obtenerListaDeMotivosDeRetiro();
...
const obtenerMotivoDeRetiroIndividual = async (req, res) => {
  ...
  try {
    const motivoRetiro =
      await motivoRetiroService.obtenerMotivoDeRetiroIndividual(motivoRetiroId);
...
const registrarMotivoDeRetiro = async (req, res) => {
  const { nombreMotivo, descripcionMotivo } = req.body;
  ...
  try {
    const motivoRetiroCreado =
      await motivoRetiroService.registrarMotivoDeRetiro(nuevomotivoRetiro);
    res
      .status(200)
      .send({
        status: "OK",
        data: `Se ha registrado el motivoRetiro '${motivoRetiroCreado}' de forma satisfactoria.`,
      });
  } catch (error) {
    res
      .status(error?.status || 500)
...
const actualizarMotivoDeRetiro = async (req, res) => {
  ...
  try {
    const motivoRetiroActualizado =
      await motivoRetiroService.actualizarMotivoDeRetiro(motivoRetiroId, body);
    res.send({
      status: "OK",
      data: `Se ha actualizado la informacion del motivo de retiro '${motivoRetiroActualizado}' de forma satisfactoria.`,
    });
  } catch (error) {
    res
      .status(error?.status || 500)
...
const borrarMotivoDeRetiro = async (req, res) => {
  ...
  try {
    ...
    res
      ...
      .send({
        status: "OK",
        data: `El motivoRetiro con el id '${motivoRetiroId}' se ha eliminado con exito.`,
      });
  } catch (error) {
    res
      .status(error?.status || 500)
