...
const actualizarPuesto = async (req, res) => {
  ...
  try {
    const puestoActualizado = await puestoService.actualizarPuesto(
      puestoId,
      body
    );
    res.send({
      status: "OK",
      data: `Se ha actualizado la informacion del puesto '${puestoActualizado}' de forma satisfactoria.`,
    });
  } catch (error) {
    res
      .status(error?.status || 500)
...
const borrarPuesto = async (req, res) => {
  ...
  try {
    ...
    res
      ...
      .send({
        status: "OK",
        data: `La puesto con el id '${puestoId}' se ha eliminado con exito.`,
      });
  } catch (error) {
    res
      .status(error?.status || 500)
